...
<body class="bg-gray-100 text-gray-800 font-sans min-h-screen flex flex-col">
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        ...
        <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
            <li><a href="<?= BASE_URL ?>/index.php?url=admin/dashboard" class="text-amber-300 font-bold hover:text-amber-100">Administrace</a></li>
        <?php endif; ?>
    ...
    <?php else: ?>
        <li><a href="<?= BASE_URL ?>/index.php?url=auth/login" class="hover:text-indigo-200 transition text-sm">Přihlásit se</a></li>
        <li><a href="<?= BASE_URL ?>/index.php?url=auth/register" class="border border-indigo-400 px-3 py-1.5 rounded-md text-sm hover:bg-indigo-500 transition">Registrace</a></li>
    <?php endif; ?>
</ul>
            </nav>
        </div>
